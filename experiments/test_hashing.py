        ...
        if self.config.exp != 'extract':
            if self.config.compute_mAP:
                for postfix, codes_name in zip(postfixes, all_codes_names):
                    ...
                    if self.config.get('sub_code_eval'):
                        if self.config.sub_code_eval_setting.rand_bits != 0:
                            start_bit = self.config.sub_code_eval_setting.start_bit
                            end_bit = self.config.sub_code_eval_setting.end_bit
                            db_out[codes_name] = db_out[codes_name][:, start_bit:end_bit]
                            test_out[codes_name] = test_out[codes_name][:, start_bit:end_bit]
                        else:
                            rand_bits = self.config.sub_code_eval_setting.rand_bits
                            nbit = db_out[codes_name].size(1)
                            bit_idxs = torch.randperm(nbit)[:rand_bits]
                            db_out[codes_name] = db_out[codes_name][:, bit_idxs]
                            test_out[codes_name] = test_out[codes_name][:, bit_idxs]

                    if self.config.get('zero_mean_eval'):
                        db_mean = db_out[codes_name].mean(dim=0, keepdim=True)
                        db_out[codes_name] = db_out[codes_name] - db_mean
                        test_out[codes_name] = test_out[codes_name] - db_mean

                    if self.config.get('test_as_database'):
                        mAPs, recalls, precisions = calculate_mAP(test_out[codes_name], test_labels,
                                                                  test_out[codes_name], test_labels,
                                                                  self.config.R,
                                                                  threshold=self.config.ternary_threshold,
                                                                  dist_metric=self.config.dist_metric,
                                                                  PRs=self.config.PRs,
                                                                  remove_first_retrieved=True)
                    else:
                ...
                for postfix, codes_name in zip(postfixes, all_codes_names):
                    ...
                    if self.config.get('test_as_database'):
                        recalls, precisions, Rs = calculate_pr_curve(test_out[codes_name], test_labels,
                                                                     test_out[codes_name], test_labels,
                                                                     threshold=self.config.ternary_threshold,
                                                                     dist_metric=self.config.dist_metric,
                                                                     remove_first_retrieved=True)
