...
class BaseTrainer:
    ...
    def load_optimizer_and_scheduler(self):
        ...
        if self.config.backbone_lr_scale != 0:
            params = [{'params': list(self.model.get_backbone().parameters()),
                       'lr': self.config.backbone_lr_scale * self.config.optim.lr},
                      {'params': list(self.model.get_training_modules().parameters())}]
        else:
            if self.config.model.get('has_adapter', False):
                params = [{'params': list(self.model.get_adapter().parameters())},
                          {'params': list(self.model.get_training_modules().parameters())}]
            else:
                params = [{'params': list(self.model.get_training_modules().parameters())}]

        # only requires_grad for those trainable params (obtained above)
        ...
        if count < 1000000:
            logging.info(f'Number of trainable params: {count / (1000):.3f}K')
        else:
            logging.info(f'Number of trainable params: {count / (1000 * 1000):.3f}M')
