...
class SEMICONTrainer(BaseTrainer):
    """
    ExchNet: A Unified Hashing Network for Large-Scale Fine-Grained Image Retrieval
    """

    def __init__(self, config):
        super(SEMICONTrainer, self).__init__(config)

        self.method_params = self.config.method_params
    ...
    def inference_one_batch(self, *args, **kwargs):
        ...
        if self.inference_datakey == 'db' and len(self.dataset['db']) == len(self.B):
            ...
            with torch.no_grad():
                codes, local_features = self.model(image)
    ...
    def train_one_batch(self, *args, **kwargs):
        ...
        train_codes, local_features = self.model(image)
    ...
    def train_one_epoch(self, **kwargs):
        ...
        self.B = solve_dcc(self.B, self.U, expand_U, S, self.B.size(1), self.config.criterion.gamma)
